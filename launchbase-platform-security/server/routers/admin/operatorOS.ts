 ...
 *
 * Import paths follow launchbase-platform conventions (see marketingInbox.ts):
 *   tRPC            → ../_core/trpc
 *   getDb           → ../db
...
const DEFAULT_LEDGER: KnowledgeLedger = {
  ...
  verifiedFacts: [
    { id: "f1", fact: "Agent-stack router healthy at port 8080", source: "system", timestamp: Date.now() - 60_000, locked: true },
    { id: "f2", fact: "580/580 tests passing — 0 TypeScript errors", source: "system", timestamp: Date.now() - 3_600_000, locked: true },
    { id: "f3", fact: "Phase 2.3 Gate 4 complete — showroom runner & benchmark runs done", source: "operator", timestamp: Date.now() - 86_400_000, locked: false },
    { id: "f4", fact: "AIML credits restored — gpt-4o-2024-08-06 as champion model", source: "system", timestamp: Date.now() - 172_800_000, locked: false },
  ],
  updatedAt: Date.now(),
};
...
const DEFAULT_PLAN: ActivePlan = {
  ...
  phase: "execution",
  tasks: [
    { id: "t1", description: "Fix DB schema sync — run pnpm db:push for swarm_runs columns", status: "pending", priority: "critical", linkedRunIds: [], createdAt: Date.now(), updatedAt: Date.now() },
    { id: "t2", description: "Fix admin auth: gate dev bypass + login redirect for /admin/*", status: "pending", priority: "critical", linkedRunIds: [], createdAt: Date.now(), updatedAt: Date.now() },
    { id: "t3", description: "Build server/agent/orchestrator.ts — aimlapi brain loop with tool schemas", status: "pending", priority: "high", linkedRunIds: [], createdAt: Date.now(), updatedAt: Date.now() },
    { id: "t4", description: "Wire approval pause/resume with stateJson + pendingActionJson", status: "pending", priority: "high", linkedRunIds: [], createdAt: Date.now(), updatedAt: Date.now() },
    { id: "t5", description: "Define craft.schema.ts + critic.schema.ts + write prompt packs", status: "pending", priority: "medium", linkedRunIds: [], createdAt: Date.now(), updatedAt: Date.now() },
    { id: "t6", description: "Run Pilot #1 — Claude 3.5 Sonnet as Critic (Web x 2 + Marketing x 2)", status: "pending", priority: "medium", linkedRunIds: [], createdAt: Date.now(), updatedAt: Date.now() },
  ],
  stopConditions: [
...
const DEFAULT_MATRIX: CapabilityMatrix = {
  projectId: "launchbase-main",
  confirmedAccess: {
    workspaces: ["launchbase-main (/home/info/agent-stack/default)", "agent-stack (docker volume)"],
    credentials: ["GITHUB_TOKEN (repo scope — verified)", "AIML_API_KEY (function calling — verified)", "STRIPE_KEY (test mode — verified)"],
    tools: ["sandbox_run", "workspace_read", "workspace_write", "workspace_list", "repo_commit", "repo_open_pr", "browser_goto", "browser_click", "browser_type", "browser_screenshot"],
  },
  missingRequirements: [
    { id: "r1", requirement: "DB connection from orchestrator service", neededFor: "Agent run persistence (agent_runs table writes)", severity: "blocking" },
    { id: "r2", requirement: "ROUTER_AUTH_TOKEN in LaunchBase env", neededFor: "LaunchBase backend -> agent-stack HTTP calls", severity: "blocking" },
    { id: "r3", requirement: "ADMIN_EMAILS env var set", neededFor: "Admin auth allowlist enforcement (/admin/* routes)", severity: "blocking" },
  ],
...
export const operatorOSRouter = router({
  ...
  projects: router({
    ...
    get: protectedProcedure
      ...
      .query(({ input }) => store.projects.get(input.id) ?? null),
    ...
    create: protectedProcedure
      ...
      .mutation(({ input }) => {
        ...
        const project: Project = { id, name: input.name, description: input.description, env: input.env, tag: input.tag, createdAt: Date.now(), updatedAt: Date.now() };
        store.projects.set(id, project);
        return project;
      }),

    update: protectedProcedure
      .input(z.object({ id: z.string(), name: z.string().optional(), description: z.string().optional(), env: z.enum(["staging", "production"]).optional(), tag: z.string().optional() }))
      .mutation(({ input }) => {
  ...
  ledger: router({
    get: protectedProcedure
      ...
      .query(({ input }) => store.ledgers.get(input.projectId) ?? null),
    ...
    addFact: protectedProcedure
      .input(z.object({ projectId: z.string(), fact: z.string().min(1), source: z.enum(["operator", "system", "test"]), locked: z.boolean().default(false) }))
      .mutation(({ input }) => {
        const ledger = store.ledgers.get(input.projectId);
        if (!ledger) throw new Error("Ledger not found for project");
        const newFact = { id: `fact-${Date.now()}`, fact: input.fact, source: input.source, timestamp: Date.now(), locked: input.locked };
    ...
    addKnownIssue: protectedProcedure
      .input(z.object({ projectId: z.string(), issue: z.string().min(1), type: z.enum(["blocking", "nonBlocking"]) }))
      .mutation(({ input }) => {
        const ledger = store.ledgers.get(input.projectId);
        if (!ledger) throw new Error("Ledger not found");
        if (input.type === "blocking") ledger.knownIssues.blocking.push(input.issue);
        else ledger.knownIssues.nonBlocking.push(input.issue);
        ledger.updatedAt = Date.now();
        return { success: true };
      }),

    resolveIssue: protectedProcedure
      .input(z.object({ projectId: z.string(), issue: z.string(), type: z.enum(["blocking", "nonBlocking"]) }))
      .mutation(({ input }) => {
        const ledger = store.ledgers.get(input.projectId);
        if (!ledger) throw new Error("Ledger not found");
        if (input.type === "blocking") ledger.knownIssues.blocking = ledger.knownIssues.blocking.filter(i => i !== input.issue);
        else ledger.knownIssues.nonBlocking = ledger.knownIssues.nonBlocking.filter(i => i !== input.issue);
        ledger.updatedAt = Date.now();
        return { success: true };
      }),
  }),
  ...
  plan: router({
    get: protectedProcedure
      .input(z.object({ projectId: z.string() }))
      .query(({ input }) => store.plans.get(input.projectId) ?? null),
    ...
    replace: protectedProcedure
      ...
      .mutation(({ input }) => {
        ...
        const plan: ActivePlan = {
          ...input,
          tasks: input.tasks.map(t => ({ ...t, createdAt: t.createdAt ?? Date.now(), updatedAt: Date.now() })),
    ...
    updateTaskStatus: protectedProcedure
      .input(z.object({ projectId: z.string(), taskId: z.string(), status: z.enum(["pending", "running", "blocked", "done"]), blocker: z.string().optional() }))
      .mutation(({ input }) => {
    ...
    addTask: protectedProcedure
      .input(z.object({ projectId: z.string(), description: z.string().min(1), priority: z.enum(["critical", "high", "medium", "low"]).default("medium"), definitionOfDone: z.string().optional() }))
      .mutation(({ input }) => {
        const plan = store.plans.get(input.projectId);
        if (!plan) throw new Error("Active plan not found");
        const task = { id: `task-${Date.now()}`, description: input.description, status: "pending" as const, priority: input.priority, linkedRunIds: [] as string[], definitionOfDone: input.definitionOfDone, createdAt: Date.now(), updatedAt: Date.now() };
    ...
    transitionPhase: protectedProcedure
      .input(z.object({ projectId: z.string(), phase: z.enum(["discussion", "execution", "validation", "complete"]) }))
      .mutation(({ input }) => {
        const plan = store.plans.get(input.projectId);
        if (!plan) throw new Error("Active plan not found");
  ...
  matrix: router({
    get: protectedProcedure
      .input(z.object({ projectId: z.string() }))
      .query(({ input }) => store.matrices.get(input.projectId) ?? null),
    ...
    resolveRequirement: protectedProcedure
      ...
      .mutation(({ input }) => {
        ...
        matrix.missingRequirements = matrix.missingRequirements.filter(r => r.id !== input.requirementId);
        matrix.updatedAt = Date.now();
        return { success: true };
      }),

    addRequirement: protectedProcedure
      .input(z.object({ projectId: z.string(), requirement: z.string().min(1), neededFor: z.string().min(1), severity: z.enum(["blocking", "degraded", "optional"]).default("blocking") }))
      .mutation(({ input }) => {
        const matrix = store.matrices.get(input.projectId);
        if (!matrix) throw new Error("Matrix not found");
        const req = { id: `req-${Date.now()}`, requirement: input.requirement, neededFor: input.neededFor, severity: input.severity };
  ...
  situationBoard: protectedProcedure
    ...
    .query(({ input }) => {
      ...
      return {
        project, ledger, plan, matrix,
        summary: {
          blockingIssueCount,
          missingReqCount,
          pendingTaskCount,
          criticalTaskCount,
          readyToExecute: blockingIssueCount === 0 && missingReqCount === 0 && plan !== null && plan.phase !== "discussion",
  ...
  runs: router({
    ...
    getArtifacts: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        if (!run) throw new TRPCError({ code: "NOT_FOUND", message: "Run not found" });
        if (!run.projectId) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "Run not attached to a project" });
    ...
    getEvents: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        if (input.type) conditions.push(eq(agentEvents.type, input.type));
        ...
        return events.map(e => ({ ...e, ts: e.ts.getTime() }));
      }),
  }),
});
